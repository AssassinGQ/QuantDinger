...
const locale = {
  ...
  'menu.dashboard.regimeSwitch': 'Regime 策略切换',
  ...
  'regime.tabs.view': '查看',
  'regime.tabs.config': '配置',
  'regime.view.currentRegime': '当前 Regime',
  'regime.view.vix': 'VIX',
  'regime.view.fearGreed': 'Fear & Greed',
  'regime.view.weights': '权重',
  'regime.view.strategyGroups': '策略分组',
  'regime.view.noConfig': '暂无配置',
  'regime.view.resetCircuitBreaker': '解除熔断',
  'regime.view.resetSuccess': '熔断已解除',
  'regime.config.createMode': '创建方式',
  'regime.config.formMode': '表单配置',
  'regime.config.yamlMode': '导入 YAML',
  'regime.config.uploadYaml': '上传 YAML',
  'regime.config.yamlPreview': '解析预览',
  'regime.config.confirmSave': '确认保存',
  'regime.config.yamlParsed': 'YAML 解析成功',
  'regime.config.indicatorTitle': 'Regime 指标配置',
  'regime.config.primaryIndicator': '主指标',
  'regime.config.vixPanic': 'Panic 阈值',
  'regime.config.vixHighVol': '高波动阈值',
  'regime.config.vixLowVol': '低波动阈值',
  'regime.config.vixHint': 'VIX > panic 为恐慌，> high_vol 为高波动，< low_vol 为低波动',
  'regime.config.fgExtremeFear': '极端恐惧阈值',
  'regime.config.fgHighFear': '高恐惧阈值',
  'regime.config.fgLowGreed': '低贪婪阈值',
  'regime.config.fgHint': 'Fear & Greed 值 0-100：低为恐慌，高为贪婪',
  'regime.config.customCode': '自定义 Python',
  'regime.config.customCodeLabel': 'Python 代码',
  'regime.config.customCodePlaceholder': '# 可用变量: macro.vix, macro.dxy, macro.fear_greed\n# 必须定义 regime 或 regime_score\nregime_score = 100 - macro["fear_greed"]  # 或直接: regime = "normal"',
  'regime.config.customCodeHint': '定义 regime（直接返回 panic/high_vol/normal/low_vol）或 regime_score（0-100，按下方阈值映射）',
  'regime.config.customCodeRequired': '请填写自定义代码',
  'regime.config.verifyCode': '校验并试运行',
  'regime.config.verifySuccess': '校验成功',
  'regime.config.customScoreExtremeFear': '分数极端恐惧阈值',
  'regime.config.customScoreHighFear': '分数高恐惧阈值',
  'regime.config.customScoreLowGreed': '分数低贪婪阈值',
  'regime.config.customScoreHint': '当 regime_score 定义时：score < 极端恐惧 → panic，< 高恐惧 → high_vol，> 低贪婪 → low_vol',
  'regime.config.strategyBinding': '策略绑定',
  'regime.config.addSymbol': '添加品种',
  'regime.config.weights': '权重配置',
  'regime.config.enableMultiStrategy': '启用多策略权重分配',
  'regime.config.saved': '配置已保存',
  'regime.config.save': '保存配置',
