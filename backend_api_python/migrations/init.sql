...
-- =============================================================================
-- 10.6. K-line Points (1m/5m 数据点，读时聚合为 1H/4H/1D/1W；5m 为 1m 不可用时的回退)
-- =============================================================================
CREATE TABLE IF NOT EXISTS qd_kline_points (
    ...
    volume DECIMAL(20,8) NOT NULL DEFAULT 0,
    created_at TIMESTAMP DEFAULT NOW(),
    PRIMARY KEY (market, symbol, time_sec, interval_sec)
);
...
-- =============================================================================
-- 17.1. IndexETF metadata + seed (Phase 20-A)
--   Adds is_inverse / leverage / underlying_index columns and the first batch
--   of 18 index ETFs across US / inverse / A-share / HK / cross-border buckets.
--   Mirrors backend_api_python/migrations/0058_qd_market_symbols_etf.sql so
--   that fresh deployments and incremental migrations stay in sync.
-- =============================================================================
