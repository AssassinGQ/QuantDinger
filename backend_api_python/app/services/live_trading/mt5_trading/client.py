...
class MT5Client(BaseStatefulClient):
    """
    MetaTrader 5 Trading Client
    ...
    def place_market_order(
        ...
        **kwargs,
        ...
        `quantity` is the BaseStatefulClient-standard parameter.
        `volume` is kept for backward compatibility — if `quantity` is 0,
        `volume` is used instead.
        """
        vol = quantity or volume
        if vol <= 0:
            return LiveOrderResult(success=False, message="volume must be positive",
        ...
        try:
            ...
            if symbol_info is None:
                return LiveOrderResult(success=False, exchange_id=self.engine_id,
            ...
            return LiveOrderResult(success=False, message=str(e),
                               exchange_id=self.engine_id)
        ...
        **kwargs,
        ...
        if vol <= 0:
            return LiveOrderResult(success=False, message="volume must be positive",
                               exchange_id=self.engine_id)
        if price <= 0:
            return LiveOrderResult(success=False, message="price must be positive",
                               exchange_id=self.engine_id)
