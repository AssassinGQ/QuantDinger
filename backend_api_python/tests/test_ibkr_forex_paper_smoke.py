...
def _make_qualify_for_pair(symbol: str, con_id: int, local_symbol: str):
    """qualifyContractsAsync mutates Forex contract in place (see test_ibkr_client.TestQualifyContractForex)."""
...
def _run_open_close_cycle(
    ...
    symbol: str,
    qty: float,
) -> tuple[LiveOrderResult, LiveOrderResult]:
    ...
    side_buy = client.map_signal_to_side("open_long", market_category="Forex")
    side_sell = client.map_signal_to_side("close_long", market_category="Forex")
...
def test_forex_paper_smoke_eurusd_uc_sa_smk_01(patched_records):
    ...
    r1, r2 = _run_open_close_cycle(client, "EURUSD", 10000.0)
...
def test_forex_paper_smoke_gbpjpy_uc_sa_smk_02(patched_records):
    ...
    r1, r2 = _run_open_close_cycle(client, "GBPJPY", 10000.0)
    assert r1.success and r2.success


@patch("app.services.live_trading.ibkr_trading.client.ib_insync", _make_mock_ib_insync())
def test_forex_paper_smoke_xagusd_uc_sa_smk_03(patched_records):
    """UC-SA-SMK-03: XAGUSD metal pair; fake conId 87654321 / XAGUSD local symbol."""
    client = _make_client_with_mock_ib()
    _wire_ib_events(client._ib)
    client._ib.qualifyContractsAsync = _make_qualify_for_pair("XAGUSD", 87654321, "XAGUSD")
    client._events_registered = False
    client._register_events()

    r1, r2 = _run_open_close_cycle(client, "XAGUSD", 10000.0)
    assert r1.success and r2.success
